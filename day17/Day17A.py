...
minHeat = [[[[float("inf") for qq in range(4)] for q in range(4)] for i in j] for j in grid]
minHeat[0][0][RIGHT][3] = 0
minHeat[0][0][DOWN][3] = 0
# heat, r, c, dir, number of straight blocks remaining
heappush(pq, (0, 0, 0, RIGHT, 3))
heappush(pq, (0, 0, 0, DOWN, 3))
seen = set()
while pq:
    tmp = heappop(pq)
    heat = tmp[0]
    ...
    if left: ndirs.append(dir)
    for ndir in ndirs:
        nr = r+drs[ndir]
        nc = c+dcs[ndir]
        nleft = left-1 if dir == ndir else 2
